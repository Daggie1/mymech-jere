                ...
                <div class="card mt-2">
                    <div class="card-body">
                        ...
                        <div class="payment @if(empty($is_address_required) === false) disabledarea @endif">
                            @php $stripFee =13
 //round(($order->price - $order->discount)*8/100) + config('settings.stripe_trans_fee_flat'), 2)
                            @endphp
                            @php  $stripeTotal = 13
//round($stripFee + ($order->price - $order->discount), 2)
                            @endphp
                            @php $paypalFee =14
 //round(($order->price - $order->discount)*(config('settings.paypal_trans_fee_percentage')/100) + config('settings.paypal_trans_fee_flat'), 2)
                            @endphp
                            @php  $paypalTotal =14
 //round($paypalFee + ($order->price - $order->discount), 2)
                            @endphp
                            <h5 class="card-title border-bottom py-2">
                            ...
                            <ul class="nav nav-tabs nav-linetriangle">
                                <li class="nav-item border-right-blue-grey border-right-lighten-5">
                                    <a class="nav-link active" href="#stripe-payment" data-toggle="tab" >
                                        <img src="{{{ asset('images/mpesa.png') }}}" class="payment-logo">
...
{{--                                            </li>--}}
                                            <li class="list-group-item d-flex justify-content-between">
                                                ...
                                                <strong class="font-medium-3">
                                                    {{{ config('settings.site_currency') }}}{{{ round($order->price - $order->discount) }}}
                                                </strong>
                                            </li>
                                       </ul>
                                    </div>
...
{{--                                              data-zip-code="false">--}}
{{--                                            </script>--}}
                                            <input type="hidden" name="type" value="mpesa" />
                                            <div class="form-group">
                                                <label>Phone Number</label>
                                            <input type="text" name="phone" placeholder="2547..."  class="form-control" required="required"/>
                                            </div>
                                            <input type="submit" value="Submit" class="btn btn-block btn-success form-control">

                                            <p class=" text-center"><img src="{{{ asset('images/mpesa_pay.jpg') }}}" height="50" width="100%"></p>
                                        </form>
                                    </div>
